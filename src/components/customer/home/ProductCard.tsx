...
const ProductCard: React.FC<Product> = (product) => {
    ...
    const renderDiscount = () => {
        ...
        return (
            <div className="flex space-x-2 text-sm">
                ...
                <span
                    className="text-error">-{calculateDiscountPercentage(product.price, product.discountPrice)}%
                </span>
            </div>
        )
    }
    ...
    return (
        <Card className="py-0  overflow-hidden gap-2  shadow-xl relative group">
            <div
                className="absolute  space-x-2 w-full flex-center transition-all  h-full opacity-0 bg-black/40 group-hover:opacity-100 "
            >
                <Button>
                    <Heart/>
                </Button>
            ...
            <div className="max-w-68  h-52 overflow-hidden rounded-2xl">
                <img className="w-full h-full  object-cover" src={product.thumbnail} alt={product.id}/>
            </div>
